...
public class LDPService {
	...
	private void updateMetaRdf(byte[] content,String about,String contentType,String headerHost){
		try {
			String rdfAbout = about.endsWith("/")?about+"meta":about+"/meta";
			String host = headerHost.endsWith("/")?headerHost:headerHost+"/";
			host = host.startsWith("http://")?host:"http://"+host;  
			...
			if(eTag!=null){
				...
				if(recordsList.size()>0){
					...
					if(contentToUpdate.getResourceType()==BygleSystemUtils.RESOURCE_TYPE_BINARY){
						...
						//String invalidProperty = findInvalidProperty(contentToUpdate.getContent(),joinUrl(records.getHost(), records.getRdfAbout()));
						if (readOnlyPropertiesChanged!=null) {
							...
							HttpHeaders headers = new HttpHeaders();
							headers.add("Link", "<"+invalidProperty+">; rel=\""+LDPVoc.constrainedBy.stringValue()+"\"");
							return new ResponseEntity<String>("you can't modify the read only property "+readOnlyPropertiesChanged,headers,HttpStatus.CONFLICT);
						}*/else{
...
//	private String findInvalidProperty(byte[] content,String uri) throws DocumentException {
...
//	}
	 ...
	 * 
	 * 	<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:bygle="http://www.bygle.org/bygle/">
	 * 	  <bygle:Patch>
	 * 	    <bygle:update rdf:parseType="Literal"><bygle:path><![CDATA[/rdf:RDF/ldp:BasicContainer[1]/dcterms:title/text()]]></bygle:path><bygle:value>A very simple container 2</bygle:value></bygle:update>
	 * 	    <bygle:insert rdf:parseType="Literal"><bygle:path><![CDATA[/rdf:RDF/ldp:BasicContainer[1]/dc:title/text()]]></bygle:path><bygle:value>Nuovo titolo</bygle:value></bygle:insert>
	 * 	    <bygle:delete rdf:parseType="Literal"><bygle:path><![CDATA[/rdf:RDF/ldp:BasicContainer[1]/ldp:contains[@rdf:resource='http://www.bygle.com/alice/member1']]]></bygle:path></bygle:delete>
	 * 	    <bygle:insert rdf:parseType="Literal"><bygle:path><![CDATA[/rdf:RDF/ldp:BasicContainer[1]/ldp:contains/@rdf:resource]]></bygle:path><bygle:value>http://www.bygle.com/alice/member4</bygle:value></bygle:insert>
	 * 	  </bygle:Patch>
     *	</rdf:RDF>
	 * 
	 * 
	 ...
	 * 
	 * [ a             bygle:Patch ;
	 *   bygle:delete  "<bygle:path xmlns:bygle=\"http://www.bygle.org/bygle/\">/rdf:RDF/ldp:BasicContainer[1]/ldp:contains[@rdf:resource='http://www.bygle.com/alice/member1']</bygle:path>"^^rdf:XMLLiteral ;
	 *   bygle:insert  "<bygle:path xmlns:bygle=\"http://www.bygle.org/bygle/\">/rdf:RDF/ldp:BasicContainer[1]/ldp:contains/@rdf:resource</bygle:path><bygle:value xmlns:bygle=\"http://www.bygle.org/bygle/\">http://www.bygle.com/alice/member4</bygle:value>"^^rdf:XMLLiteral , "<bygle:path xmlns:bygle=\"http://www.bygle.org/bygle/\">/rdf:RDF/ldp:BasicContainer[1]/dc:title/text()</bygle:path><bygle:value xmlns:bygle=\"http://www.bygle.org/bygle/\">Nuovo titolo</bygle:value>"^^rdf:XMLLiteral ;
	 *   bygle:update  "<bygle:path xmlns:bygle=\"http://www.bygle.org/bygle/\">/rdf:RDF/ldp:BasicContainer[1]/dcterms:title/text()</bygle:path><bygle:value xmlns:bygle=\"http://www.bygle.org/bygle/\">A very simple container 2</bygle:value>"^^rdf:XMLLiteral
	 * ] .
	 * */
